...
import cryption.rsa.prng.insecure;
import cryption.rsa.bigint;
import cryption.rsa.hash.d;
import cryption.rsa.hash.sha1;
    ...
    */
    ...
    private bool millerRabinPrimeTest(BigInt randomCandidate, uint tries)
    {
        ...
        for(uint i = 0; i < tries; i++)
        {
            ...
            do
            {
                apick = randomBigInt(cast(uint)rcmin1.bitLength);
            }
...
class RSA
{
    ...
    */
    ...
    private ubyte[] depadOAEM(Hash hashfn, ubyte[] padded_message)
    {
        ...
        ubyte[] dbMask;
        OAEP_MGF(hashfn, maskedSeed, cast(int)maskedDb.length, dbMask);
        maskedDb[] = dbMask[] ^ maskedDb[];
