module cryption.rsa.biguintcore;
...
public:
    ...
    size_t trailingZeroBits() pure const
    {
        ...
        while( (leastsig = peekUlong(cast(int)i++) ) == 0)
        {
    ...
    size_t bitLength() pure const
    {
        ...
        uint last = peekUint(cast(int)(length-1));
...
public:
    ...
    */
    ...
    bool fromUbyteArray(ubyte[] d)
    {
        ...
        int numBytes = cast(int)(d.length - firstNonZero);
        int numWholeWords = cast(int)(numBytes / BigDigit.sizeof);
    ...
    */
    ...
    {
        BigUint c = BigUint([1]);
        for (int block = cast(int)(exp.data.length-1); block >= 0; --block)
        {
...
void mulInternal(BigDigit[] result, const(BigDigit)[] x, const(BigDigit)[] y)
{
    ...
    if (2*y.length*y.length <= x.length*x.length)
    {
        ...
        if (extra * extra * 2 < y.length*y.length)
        {
            ...
            chunksize += cast(ulong)(extra / cast(double)numchunks);
