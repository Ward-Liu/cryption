...
private import cryption.rsa.biguintcore;
...
public:
    ...
    ubyte[] toUbyteArrayDim(size_t bytelength) const
    {
        ...
        {
            result[start_from+4*i..start_from+4*(i+1)] = nativeToBigEndian!uint(data.peekUint(cast(int)(uintLength-i-1)));
        }
    ...
    ubyte[] toUbyteArray() const
    {
        ...
        foreach(i; 0..uintlength)
        {
            result[4*i..4*(i+1)] = nativeToBigEndian!uint(data.peekUint(cast(int)(uintLength-i-1)));
        }
